      ...
      <div id="dialogbox" class="dialogbox" style="display:none;">\
        ...
        <div class="dialogboxClose"></div>\
						...
						init : function(settings) {
							if (!$(".dialogbox").length)
								$('body').append(
										$.dialogbox.settings.dialogboxHtml);
						...
						prompt : function(data) {
							var content = data ? data.content ? data.content
									...
									: 2000
									: '';
							$(".dialogPrompt").fadeOut(function() {
								$(this).remove()
							});
							$('body').append(
									'<div class="dialogPrompt shake">'
											+ content + '</div>');
							$.dialogbox.position($('.dialogPrompt'));
							if (time) {
								clearTimeout(promptAuto);
								promptAuto = setTimeout(function() {
									$('.dialogPrompt').stop(true, true)
											.fadeOut(function() {
												$(this).remove()
											});
								}, time);
						...
						reveal : function(data) {
							var title = data ? data.title ? data.title : ''
									...
									: '', width = data ? data.width > 0 ? data.width
									: $(window).width() < 1200 ? $(window)
											.width() - 150 : 1100
									: '', height = data ? data.height > 0 ? data.height
									: $(window).height() - 250
									: '', time = data ? data.time ? data.time
									: '' : '', closeBtn = data ? data.closeBtn ? data.closeBtn
									: false
									: false;
							var maxWidth = data && data.maxWidth ? (data.maxWidth + 'px') : '';
							...
							if (data && data.type == 'msg') {
								...
								if (data.content) {
									/*
									 * tipsHtml = '<div class="dialogTips"><img
									 * src="'+icon+'" />'+content+'</div>';
									 *//*
										 * Quang comment
										 */
									tipsHtml = '<div class="dialogTips">'
											+ content + '</div>';
								}
								...
								if (data.btn && data.btn instanceof Array) {
									...
									for (var lenBtn = 0; lenBtn < data.btn.length; lenBtn++) {
										...
										var btnKlass = btnClass
												.hasOwnProperty(lenBtn) ? btnClass[lenBtn]
												: 'button-default';
										if (data.color
												&& data.color instanceof Array
												&& data.color.length >= lenBtn) {
											var btnClassKey = data.color[lenBtn];
											if (btnClass
													.hasOwnProperty(btnClassKey))
												btnKlass = btnClass[btnClassKey];
										}
										btnHtml += '<a class=" ' + btnKlass
												+ '">' + btnName + '</a>';
									}
									btnHtml += '</div>';
								}
										...
										.on(
												...
												function() {
													var aIndex = $(this)
															.index();
													if (data.call
															&& data.call instanceof Array
															&& data.call.length >= aIndex) {
								...
								var iframeHtml = '<iframe width="' + width
										+ '" height="' + height + '" src="'
										+ uri + '" frameborder="no"></iframe>';
								if (content)
									iframeHtml = '<div class="dialogTopTips">'
											+ content + '</div>' + iframeHtml;
								content = iframeHtml;
							...
							$('.dialogboxBody').css('max-width', maxWidth);
							if (title) {
						...
						close : function() {
							...
							if ($.dialogbox.closeCallback
									&& $.dialogbox.closeCallback instanceof Function) {
						...
						/* 显示位置 Quang comment */
						/*
						 * position:function(obj){ var left = $(window).width() /
						 * 2 - ($(obj).outerWidth() / 2) +
						 * $(window).scrollLeft(); var top = $(window).height() /
						 * 2 - ($(obj).outerHeight() / 2) +
						 * $(window).scrollTop(); if(top <
						 * $(window).scrollTop()) top= $(window).scrollTop() +
						 * 10; $(obj).css({left:left,top:top}).fadeIn();
						...
						position : function(obj) {
							...
							$(".dialogboxOverlay").css('width', '').css({
						...
						showOverlay : function(data) {
							...
							if (!$(".dialogboxOverlay").length)
								$("body").append(
										'<div class="dialogboxOverlay"></div>');
							if (data && data.overlayOpacity)
								overlay = data.overlayOpacity;
							$(".dialogboxOverlay").css({
								'opacity' : overlay,
								height : $(document).height(),
								width : $(document).width(),
								'z-index' : 100000
						...
						hideOverlay : function() {
							$(".dialogboxOverlay").fadeOut(function() {
								$(this).remove()
							});
						},
						/* 拖动 */
						move : function() {
							...
							$(document).mousemove(
									function(e) {
										...
										mouseX = e.pageX;
										mouseY = e.pageY;
										var left = parseInt(mouseX - defaultX)
												+ divLeft;
										var top = parseInt(mouseY - defaultY)
												+ divTop;
										if (left < 0)/* 防止拖动出左边 */
											left = 0;
										if (top < 0)/* 防止拖动出顶部 */
											top = 0;
										if (left + box.outerWidth() > $(
												document).width())/* 防止拖动出右边 */
											left = $(document).width()
													- box.outerWidth();
										if (top + box.outerHeight() > $(
												document).height())/* 防止拖动出底边 */
											top = $(document).height()
													- box.outerHeight();
										if (isClick) {/* 确认鼠标按住 */
											box.css({
